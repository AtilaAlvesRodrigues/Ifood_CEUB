...
const specialOffersData = [
    {
        id: 5,
        image: null,
        name: "Restaurante Bom Preço",
        type: "Variada",
        price: "$$",
        rating: 4.3,
        reviews: "300+",
        time: "35-50 min",
        offers: [{ type: "free-shipping", text: "Frete Grátis" }],
    },
    {
        id: 6,
        image: null,
        ...
        reviews: "750+",
        time: "45-60 min",
        offers: [{ type: "discount", text: "15% OFF" }],
    },
    {
        id: 7,
        image: null,
        ...
        time: "20-35 min",
        offers: [
            { type: "free-shipping", text: "Frete Grátis" },
            { type: "discount", text: "Combo Especial" },
        ],
    },
    {
        id: 8,
...
function createRestaurantCard(restaurantData) {
    ...
    cardElement.querySelector(".rating-value").textContent =
        restaurantData.rating;
    cardElement.querySelector(".review-count").textContent =
...
function renderRestaurants(restaurants, containerElement) {
    ...
    if (restaurants.length === 0) {
        containerElement.innerHTML =
            "<p>Nenhum restaurante encontrado para esta seleção.</p>"; // Show message if no results
        return;
    }
...
function filterRestaurants(searchTerm, category, dataSet) {
    ...
    return dataSet.filter((restaurant) => {
        ...
        const categoryMatch =
            category === "all" ||
            restaurant.type.toLowerCase() === category.toLowerCase();
...
function handleFilterAndSearch() {
    ...
    const filteredPopular = filterRestaurants(
        currentSearchTerm,
        currentCategory,
        popularRestaurantsData
    );
    const filteredOffers = filterRestaurants(
        currentSearchTerm,
        currentCategory,
        specialOffersData
    );
...
function handleCardClick(event) {
    ...
    if (card) {
        ...
        alert(
            `Você clicou no restaurante: ${restaurantName} (ID: ${
                restaurantId || "N/A"
            })`
        );
